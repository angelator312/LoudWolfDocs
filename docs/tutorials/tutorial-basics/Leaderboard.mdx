...
First we have to remember that everything for leaderbords is in [`LoudWolf.Scores`](../../api/Scores.mdx) and that every function have an async variation caleed `func_name`_async
...
We also have [`get_scores(x?:int)`](../../api/Scores#get-highest-scores)
